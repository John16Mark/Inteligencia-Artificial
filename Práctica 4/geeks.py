...
# This function gets the neighbour of the current state having
# the least objective value amongst all neighbours as
# well as the current state.
def getNeighbour(board, state):

	# Declaring and initializing the optimal board and state with
	...
	for i in range(N):
		for j in range(N):
			...
			if (j != state[i]) :
				...
				# Initializing temporary neighbour with the
				# current neighbour.
				NeighbourState[i] = j;
	...
	# Copying the optimal board and state thus found to the current
	# board and, state since c+= 1 doesn't allow returning multiple values.
